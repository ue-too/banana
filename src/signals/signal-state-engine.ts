...
import type { BlockId, SignalAspect, SignalId } from './types';
...
export class SignalStateEngine {
    ...
    update(
        ...
        for (const [blockId, block] of blocks) {
            ...
            for (const entry of block.segments) {
                const trainsOnSeg = occupancyRegistry.getTrainsOnSegment(
                    entry.segmentNumber
                );
    ...
    getDistanceToRestrictiveSignal(
        ...
        while (totalDistance < MAX_LOOKAHEAD) {
            ...
            const nextJointDir = exitJoint.direction.reverseTangent.has(
                entryJointNumber
            )
                ? ('tangent' as const)
            ...
            const nextSeg = trackGraph.getTrackSegmentWithJoints(
                next.curveNumber
            );
            if (!nextSeg) break;
