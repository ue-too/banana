...
export class BlockSignalManager {
    ...
    computeBlockSegments(
        ...
        while (
            ...
            for (const nextJointNum of nextJoints) {
                ...
                if (segNum === exitSig.segmentNumber) {
                    ...
                    const exitSegData =
                        trackGraph.getTrackSegmentWithJoints(segNum);
                    if (exitSegData) {
                        foundSegDir =
                            exitSegData.t0Joint === current.jointNumber
                                ? 'tangent'
                                : 'reverseTangent';
                    }
                    break;
        ...
        if (entrySig.direction === 'tangent') {
            result.push({
                segmentNumber: entrySig.segmentNumber,
                fromT: entrySig.tValue,
                toT: 1,
            });
        } else {
            result.push({
        ...
        if (foundSegDir === 'tangent') {
            result.push({
                segmentNumber: exitSig.segmentNumber,
                fromT: 0,
                toT: exitSig.tValue,
            });
        } else {
