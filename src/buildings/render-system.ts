...
export class BuildingRenderSystem {
    ...
    constructor(
        ...
        buildingManager.onAdd(this._onAdd.bind(this), {
            signal: this._abortController.signal,
        });
        buildingManager.onDelete(this._onDelete.bind(this), {
            signal: this._abortController.signal,
        });
        buildingManager.onUpdate(this._onUpdate.bind(this), {
            signal: this._abortController.signal,
        });
    }
    ...
    private _onUpdate(id: number, data: BuildingData): void {
        ...
        const key = buildingKey(id);
        const roofBand = rooftopBandIndex(
            data.elevation as number,
            data.height
        );
    ...
    private _updateAll(): void {
        for (const [, record] of this._records) {
            ...
            drawBuildingShadow(
                record.shadowGraphics,
                record.data.vertices,
                record.data.elevation as number,
                record.data.height,
                this._sunAngle,
                this._baseShadowLength
            );
...
const drawBuilding = (graphics: Graphics, baseVertices: Point[]): void => {
    drawPolygon(graphics, baseVertices);
