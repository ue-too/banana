...
const BOGIE_COLORS: number[] = [
    0xff0000, 0x00ff00, 0x0000ff, 0xffff00, 0x800080, 0xffa500, 0xffc0cb,
    0xa52a2a, 0x808080, 0x000000, 0xffffff,
];
...
function getGangwayGeometries(
    ...
    for (let k = 0; k + 1 < cars.length; k++) {
        ...
        const overhangA =
            (cars[k].flipped ? cars[k].edgeToBogie : cars[k].bogieToEdge) -
            CAR_GAP;
        const overhangB =
            (cars[k + 1].flipped
                ? cars[k + 1].bogieToEdge
                : cars[k + 1].edgeToBogie) - CAR_GAP;
...
function getCarGeometries(
    ...
    for (let k = 0; 2 * k + 1 < positions.length; k++) {
        ...
        const edgeToBogie =
            (car.flipped ? car.bogieToEdge : car.edgeToBogie) - CAR_GAP;
        const bogieToEdge =
            (car.flipped ? car.edgeToBogie : car.bogieToEdge) - CAR_GAP;
...
export class TrainRenderSystem {
    ...
    private _customCarTextures: Map<
        string,
        { full: Texture; rear: Texture; front: Texture }
    > = new Map();
    ...
    /** Set the transfer manager. Called during app init. */
    setTransferManager(manager: TransferManager): void {
        this._transferManager = manager;
    }

    /** Set the source/sink ticker. Called during app init. */
        ...
        for (const { id, train } of placed) {
            ...
            for (let i = 0; i < positions.length; i++) {
                ...
                const bandIndex =
                    i === 0
                        ? headBand
                        : (this._resolveBandIndex(positions[i]) ?? headBand);
                if (bandIndex !== null) {
    ...
    private _updateActualCars(placed: readonly PlacedTrainEntry[]): void {
        ...
        for (const { id, train } of placed) {
            ...
            for (let i = 0; i < halves.length; i++) {
                ...
                const customTex = car
                    ? this._getCustomCarTextures(car.id)
                    : null;
                ...
                const tex = customTex
                    ? isRearHalf
                        ...
                        : customTex.front
                    : isRearHalf
                      ? defaultTextures.rear
                      : defaultTextures.front;
                ...
                const bandIndex = this._resolveBandIndex(
                    positions[h.bogiePositionIndex]
                );
                if (bandIndex !== null) {
                    this._worldRenderSystem.addToBand(
                        carKey(id, i),
    ...
    private _updateGangways(placed: readonly PlacedTrainEntry[]): void {
        ...
        for (const { id, train } of placed) {
            ...
            for (let i = 0; i < geoms.length; i++) {
                ...
                sprite.scale.set(
                    g.length / texture.width,
                    GANGWAY_WIDTH / texture.height
                );
                sprite.visible = true;

                const bandIndex = this._resolveBandIndex(
                    positions[g.bogiePositionIndex]
                );
                if (bandIndex !== null) {
                    this._worldRenderSystem.addToBand(
                        gangwayKey(id, i),
                    ...
                    this._worldRenderSystem.setOrderInBand(
                        gangwayKey(id, i),
                        0
                    );
                }
    ...
    private _syncGangwayPoolForTrain(
        trainId: number,
        count: number,
        texture: Texture
    ): void {
    ...
    private _syncCouplerPoolForTrain(
        trainId: number,
        count: number,
        texture: Texture
    ): void {
    ...
    private _getOrCreateCarHalfTextures(): {
        rear: Texture;
        front: Texture;
    } | null {
    ...
    private _getCustomCarTextures(
        carId: string
    ): { rear: Texture; front: Texture } | null {
        const cached = this._customCarTextures.get(carId);
        if (cached) return cached;

        if (!this._carImageRegistry || !this._carImageRegistry.has(carId))
            return null;
    ...
    private _syncPreviewPool(count: number): void {
        ...
        for (let i = 0; i < this._previewGraphicsPool.length; i++) {
            this._previewGraphicsPool[i].visible =
                this._showBogies && i < count;
        }
    }
}
