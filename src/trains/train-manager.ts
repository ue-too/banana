...
export class TrainManager {
    private _internalTrainManager: GenericEntityManager<Train> =
        new GenericEntityManager<Train>(10);
    private _placedTrains: PlacedTrainEntry[] = [];
    private _selectedIndex = 0;
    private _listeners: (() => void)[] = [];
    private _observable: Observable<[number, { type: TrainChangeType }]> =
        new SynchronousObservable<[number, { type: TrainChangeType }]>();
    ...
    decoupleTrainAtCar(
        ...
        const newTrain = train.decoupleAtCar(
            headCarIndex,
            tailCarIndex,
            inherit
        );
    ...
    coupleTrains(match: ProximityMatch): CoupleResult {
        ...
        if (endA === 'tail' && endB === 'head') {
            keepId = match.trainA.id;
            removeId = match.trainB.id;
            keepTrain = trainA;
            removeTrain = trainB;
        } else if (endA === 'head' && endB === 'tail') {
            keepId = match.trainB.id;
            removeId = match.trainA.id;
            keepTrain = trainB;
            removeTrain = trainA;
        ...
        if (
            keepTrain.formation.depth + removeTrain.formation.depth >=
            MAX_FORMATION_DEPTH
        ) {
    ...
    subscribeToChanges(
        listener: (id: number, type: TrainChangeType) => void
    ): () => void {
