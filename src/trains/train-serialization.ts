...
function serializeCar(car: Car): SerializedCar {
    ...
    return {
        ...
        ...(car.couplerLength !== 0
            ? { couplerLength: car.couplerLength }
            : {}),
...
function serializeFormation(formation: Formation): SerializedFormation {
    const children: SerializedFormationChild[] = formation.children.map(
        child => {
            if (child.depth === 0) {
...
export function serializeTrainData(
    ...
    const cars: SerializedCar[] = Array.from(carById.values()).map(
        serializeCar
    );
    const formations: SerializedFormation[] = Array.from(
        formationById.values()
    ).map(serializeFormation);
...
export function validateSerializedTrainData(
    ...
    if (!Array.isArray(obj.carStockIds)) {
        return {
            valid: false,
            error: 'Missing or invalid "carStockIds" array',
        };
    }
    if (!Array.isArray(obj.formationManagerIds)) {
        return {
            valid: false,
            error: 'Missing or invalid "formationManagerIds" array',
        };
    }
    if (!Array.isArray(obj.placedTrains)) {
        return {
            valid: false,
            error: 'Missing or invalid "placedTrains" array',
        };
    }
    for (let i = 0; i < (obj.cars as unknown[]).length; i++) {
        const c = (obj.cars as unknown[])[i] as Record<string, unknown>;
        if (typeof c?.id !== 'string')
            return { valid: false, error: `cars[${i}].id must be a string` };
        if (!Array.isArray(c?.bogieOffsets))
            return {
                valid: false,
                error: `cars[${i}].bogieOffsets must be an array`,
            };
        if (typeof c?.edgeToBogie !== 'number')
            return {
                valid: false,
                error: `cars[${i}].edgeToBogie must be a number`,
            };
        if (typeof c?.bogieToEdge !== 'number')
            return {
                valid: false,
                error: `cars[${i}].bogieToEdge must be a number`,
            };
        if (typeof c?.flipped !== 'boolean')
            return {
                valid: false,
                error: `cars[${i}].flipped must be a boolean`,
            };
    }
    for (let i = 0; i < (obj.formations as unknown[]).length; i++) {
        ...
        if (typeof f?.id !== 'string')
            return {
                valid: false,
                error: `formations[${i}].id must be a string`,
            };
        if (!Array.isArray(f?.children))
            return {
                valid: false,
                error: `formations[${i}].children must be an array`,
            };
        if (typeof f?.flipped !== 'boolean')
            return {
                valid: false,
                error: `formations[${i}].flipped must be a boolean`,
            };
    }
    for (let i = 0; i < (obj.placedTrains as unknown[]).length; i++) {
        const pt = (obj.placedTrains as unknown[])[i] as Record<
            string,
            unknown
        >;
        if (typeof pt?.id !== 'number')
            return {
                valid: false,
                error: `placedTrains[${i}].id must be a number`,
            };
        const pos = pt?.position as Record<string, unknown> | undefined;
        if (
            !pos ||
            typeof pos?.trackSegment !== 'number' ||
            typeof pos?.tValue !== 'number' ||
            (pos?.direction !== 'tangent' &&
                pos?.direction !== 'reverseTangent')
        ) {
            return {
                valid: false,
                error: `placedTrains[${i}].position must be { trackSegment, tValue, direction }`,
            };
        }
        if (typeof pt?.formationId !== 'string')
            return {
                valid: false,
                error: `placedTrains[${i}].formationId must be a string`,
            };
        if (pt?.maxSpeed !== undefined) {
            if (
                typeof pt.maxSpeed !== 'number' ||
                !Number.isFinite(pt.maxSpeed) ||
                pt.maxSpeed < 0
            ) {
                return {
                    valid: false,
                    error: `placedTrains[${i}].maxSpeed must be a finite number >= 0 when present`,
                };
            }
        }
    }
