...
export function generateProceduralTrackPath(
    ...
    const ok = trackGraph.createNewTrackSegment(points[0], points[1], [mid0]);
    if (!ok) return 0;
    ...
    for (let i = 2; i <= segmentCount; i++) {
        ...
        const ok = trackGraph.branchToNewJoint(last.endJointNumber, points[i], [
            mid,
        ]);
        if (!ok) break;
        created += 1;
...
export function generateParallelTracks(
    ...
    const { count, length = 100, gap = 5, startX = 0, startY = 0 } = options;
