...
export function fillDepression(
    ...
    if (seedCol < 0 || seedCol >= vx || seedRow < 0 || seedRow >= vy)
        return NaN;
...
export function generateWater(
    ...
    const floodLevel =
        options.floodLevel ?? sorted[Math.floor(sorted.length * 0.15)];
 ...
 * ```
 */
export function createHillyWithWater(
