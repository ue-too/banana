...
export class TerrainRenderSystem {
    ...
    private _destroyVisuals(): void {
        ...
        for (let i = 0; i < this._occlusionMeshes.length; i++) {
            ...
            if (mesh) {
                const oc =
                    this._worldRenderSystem.getTerrainOcclusionContainer(i);
    ...
    private _buildBaseMesh(): void {
        ...
        this._baseCanvasSource = new CanvasSource({
            resource: canvas,
            resolution: 1,
        });
    ...
    private _buildWaterMesh(): void {
        ...
        this._waterCanvasSource = new CanvasSource({
            resource: canvas,
            resolution: 1,
        });
    ...
    private _buildContourLines(): void {
        ...
        const startLevel =
            Math.ceil(minH / CONTOUR_MINOR_INTERVAL) * CONTOUR_MINOR_INTERVAL;
        const endLevel =
            Math.floor(maxH / CONTOUR_MINOR_INTERVAL) * CONTOUR_MINOR_INTERVAL;

        for (
            let level = startLevel;
            level <= endLevel;
            level += CONTOUR_MINOR_INTERVAL
        ) {
    ...
    private _buildOcclusionMeshes(): void {
        ...
        for (let bandIdx = 0; bandIdx < bandCount; bandIdx++) {
            ...
            if (mesh) {
                ...
                const oc =
                    this._worldRenderSystem.getTerrainOcclusionContainer(
                        bandIdx
                    );
                if (oc) {
                    oc.addChild(mesh);
     ...
     */
    private _buildOcclusionMeshForThreshold(
        threshold: number
    ): MeshSimple | null {
        const td = this._terrainData;
        const { cellsX, cellsY, cellSize, originX, originY } = td.config;
