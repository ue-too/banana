...
export class AutoDriver {
    ...
    constructor(state: ActiveShiftState, jdm: TimetableJointDirectionManager) {
        this._state = state;
        this._jdm = jdm;
    }
    ...
    onJointsPassed(passedJoints: { jointNumber: number }[]): void {
        ...
        for (const passed of passedJoints) {
            for (
                let i = this._state.routeJointProgress;
                i < routeJoints.length;
                i++
            ) {
    ...
    private _handleDeparting(
        ...
        if (signalStateEngine && train.position) {
            ...
            if (
                signalInfo &&
                signalInfo.aspect === 'red' &&
                signalInfo.distance < CREEP_DISTANCE
            ) {
                train.setThrottleStep('b1');
                return;
            }
    ...
    private _handleRunning(
        ...
        const signalInfo = this._getSignalInfo(
            train,
            trackGraph,
            signalStateEngine
        );
        ...
        if (signalInfo) {
            ...
            if (signalInfo.aspect === 'red') {
                if (
                    ...
                    signalInfo.distance < effectiveDistance
                ) {
                ...
                if (train.speed > cautionSpeed) {
                    const brakingDist = this._computeBrakingDistance(
                        train.speed - cautionSpeed
                    );
                    if (
                        signalInfo.distance <=
                        brakingDist * BRAKING_SAFETY_MARGIN
                    ) {
    ...
    private _handleApproaching(
        ...
        if (
            train.speed <= STOP_SPEED_THRESHOLD &&
            effectiveDistance <= CREEP_DISTANCE
        ) {
            ...
            if (
                ...
                if (signalStateEngine && train.position) {
                    const freshSignal =
                        signalStateEngine.getDistanceToRestrictiveSignal(
                            train.position,
                            trackGraph,
                            this._jdm
                        );
    ...
    private _getDistanceToStop(
        ...
        if (nextScheduledStop.platformKind === 'trackAligned') {
            ...
            const station = stationManager.getStation(
                nextScheduledStop.stationId
            );
    ...
    private _computeDistanceAlongRoute(
        ...
        const currentSeg = trackGraph.getTrackSegmentWithJoints(
            from.trackSegment
        );
