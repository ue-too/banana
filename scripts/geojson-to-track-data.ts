...
function perpendicularDist(p: Point, a: Point, b: Point): number {
    ...
    const t = Math.max(
        0,
        Math.min(1, ((p.x - a.x) * dx + (p.y - a.y) * dy) / (len * len))
    );
...
function isLineString(
    g: GeoGeometry
): g is { type: 'LineString'; coordinates: number[][] } {
    return (
        g.type === 'LineString' &&
...
function main(): void {
    ...
    const lineStrings = geojson.geometries.filter((g: GeoGeometry) =>
        isLineString(g)
    );
    const geometryStep =
        lineStrings.length <= 25
            ? 1
            : Math.max(1, Math.floor(lineStrings.length / 25));
    ...
    for (let j = 0; j < jointPositions.length; j++) {
        ...
        const raw =
            Math.round(t * (ELEVATION_MAX - ELEVATION_MIN)) + ELEVATION_MIN;
        jointElevation[j] = Math.max(
            ELEVATION_MIN,
            Math.min(ELEVATION_MAX, raw)
        );
    }
    ...
    for (let j = 0; j < jointPositions.length; j++) {
        ...
        joints.push({
            ...
            tangent,
            direction: {
                tangent: tangentJoints,
                reverseTangent: reverseTangentJoints,
            },
    ...
    const filteredJoints = joints
        ...
        .map(j => {
            ...
            return {
                jointNumber: newNum,
                position: j.position,
                connections: j.connections.map(
                    ([adj, segNum]) =>
                        [oldToNewJoint.get(adj)!, segNum] as [number, number]
                ),
                tangent: j.tangent,
                direction: {
                    tangent: j.direction.tangent.map(
                        adj => oldToNewJoint.get(adj)!
                    ),
                    reverseTangent: j.direction.reverseTangent.map(
                        adj => oldToNewJoint.get(adj)!
                    ),
                },
    ...
    const filteredSegments = segments.map((seg, idx) => ({
        ...
        elevation: {
            from: jointElevation[seg.t0Joint],
            to: jointElevation[seg.t1Joint],
        },
        gauge: GAUGE,
        splits: [] as number[],
    }));
    ...
    if (outputPath) {
        ...
        console.log(
            `Wrote ${filteredJoints.length} joints, ${filteredSegments.length} segments to ${outputPath}`
        );
    } else {
        console.log(json);
    }
