...
function makeMockJDM(
    routeJoints: { jointNumber: number }[] = []
): TimetableJointDirectionManager {
    let idx = 0;
    return {
...
function makeState(
    overrides: Partial<ActiveShiftState> = {}
): ActiveShiftState {
    return {
        assignmentId: 'a1',
        trainId: 1,
...
function makeShift(
    departureTime: WeekMs = 1000,
    arrivalTime: WeekMs = 5000
): ShiftTemplate {
    return {
        id: 's1',
        name: 'Test Shift',
        ...
        stops: [makeStop({ departureTime }), makeStop({ arrivalTime })],
        legs: [{ routeId: 'r1' }],
    };
}
...
describe('AutoDriver', () => {
    ...
    describe('onJointsPassed', () => {
        it('does nothing for empty array', () => {
            ...
            const jdm = makeMockJDM([{ jointNumber: 0 }, { jointNumber: 1 }]);
