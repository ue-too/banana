import {
    ...
    MS_PER_WEEK,
    ScheduleClock,
} from '../src/timetable/schedule-clock';
...
describe('ScheduleClock', () => {
    ...
    describe('toWeekMs', () => {
        ...
        it('wraps around at the end of the week', () => {
            ...
            const epoch =
                mondayEpoch(DayOfWeek.Sunday, 23, 59, 59) + 2 * MS_PER_SECOND;
        ...
        it('handles multiple week wraps', () => {
            const clock = new ScheduleClock();
            const epoch =
                mondayEpoch(DayOfWeek.Monday, 0, 0, 0) +
                2 * MS_PER_WEEK +
                MS_PER_HOUR;
        ...
        it('computes correctly for mid-week timestamp', () => {
            ...
            const expected =
                DayOfWeek.Wednesday * MS_PER_DAY +
                14 * MS_PER_HOUR +
                30 * MS_PER_MINUTE;
    ...
    describe('serialize / deserialize', () => {
        it('deserialize returns a functional clock regardless of input', () => {
            const restored = ScheduleClock.deserialize({
                startDay: 3,
                startHours: 9,
            });
            const epoch = mondayEpoch(DayOfWeek.Monday, 0, 0, 0);
