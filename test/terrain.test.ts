...
import {
    ...
    sampleColorRamp,
    sampleWaterColor,
} from '../src/terrain/terrain-colors';
import {
    TerrainData,
    validateSerializedTerrainData,
} from '../src/terrain/terrain-data';
import {
    createHillyWithWater,
    fillDepression,
    floodBelow,
    generateWater,
    traceRiver,
} from '../src/terrain/terrain-water';
...
describe('validateSerializedTerrainData', () => {
    ...
    it('should reject missing config', () => {
        expect(
            validateSerializedTerrainData({ heightsBase64: 'abc' }).valid
        ).toBe(false);
    });
...
describe('terrain-colors', () => {
    ...
    it('sampleColorRamp should vary with height', () => {
        ...
        expect(low.r !== high.r || low.g !== high.g || low.b !== high.b).toBe(
            true
        );
    });
...
describe('terrain-water', () => {
    ...
    describe('generateWater', () => {
        it('should populate water on hilly terrain', () => {
            const terrain = TerrainData.createHilly(config, {
                amplitude: 20,
                seed: 42,
            });
        ...
        it('should be reproducible with the same seed', () => {
            const t1 = TerrainData.createHilly(config, {
                amplitude: 20,
                seed: 42,
            });
            generateWater(t1, { seed: 7 });
            const t2 = TerrainData.createHilly(config, {
                amplitude: 20,
                seed: 42,
            });
    ...
    describe('createHillyWithWater', () => {
        it('should create terrain with both heights and water', () => {
            const terrain = createHillyWithWater(config, {
                amplitude: 20,
                seed: 42,
            });
